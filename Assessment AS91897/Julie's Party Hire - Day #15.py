...
def button():
    ...
    quit_button = Button(frame, font='bold', text= "Quit", command=quit)
    quit_button.grid(column=4, row=0)

    print_button = Button(frame, font='bold', text = "Append Details", command = append_details)
    print_button.grid(column=3, row=1)

    delete_row_button = Button(frame, font='bold', text = 'Delete Row', command = delete_row)
...
def append_details():
    ...
    try:
        ...
        customer_name_error.set("Check inputs")
        weather_error_var.set("")

        entry_customer_name.delete(0,END)
        entry_receipt_number_str.delete(0,END)
        entry_items_quantity.delete(0,END)
        entry_items_hired.delete(0,END)

def error_prevention():

    customer_name_error = Label(frame, textvariable = customer_name_error_var, fg = 'red')
    customer_name_error.grid(column=2, row=0)

    receipt_number_error = Label(frame, textvariable = receipt_number_error_var, fg = 'red')
    receipt_number_error.grid(column=2, row=1)

    items_quantity_error = Label(frame, textvariable = items_quantity_error_var, fg = 'red', width = 13)
    items_quantity_error.grid(column=2, row=2)
    
    items_hired_error = Label(frame, textvariable = items_hired_error_var, fg = 'red')
