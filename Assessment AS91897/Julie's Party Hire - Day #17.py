...
def entry_labels():
    ...
    Label(main_window, font='bold',text="Customer name").grid(column=0,row=0)
    Label(main_window, font='bold',text="Receipt number").grid(column=0,row=1)
    Label(main_window, font='bold',text="Items quantity").grid(column=0,row=2) 
...
def entries():
    ...
    entry_items_quantity.grid(column=1,row=2)
    ...
    delete_row_entry.grid(column=4, row=3)
...
def button():
    ...
    print_button.grid(column=4, row=1)

    delete_row_button = Button(main_window, font='bold', text = 'Delete Row')
    delete_row_button.grid(column=4, row=2)

def table_headers():
    row_header = Label(main_window, text='Row', font = 'bold')
...
def append_details():
    ...
    second_frame.grid(row=0,column=6)
    ...
    try:
        ...
        while name_count < counters['total_entries'] :
            ...
            Label(second_frame, text=name_count).grid(column=0,row=name_count + ROWS_ABOVE,padx=60)
            Label(second_frame, text=(input_data_col1[name_count][-1])).grid(column=1,row=name_count + ROWS_ABOVE, padx=50)           
            Label(second_frame, text=(input_data_col2[name_count][-1])).grid(column=2,row=name_count + ROWS_ABOVE, padx=50)            
            Label(second_frame, text=(input_data_col3[name_count][-1])).grid(column=3,row=name_count + ROWS_ABOVE, padx=50)
            Label(second_frame, text=(input_data_col4[name_count][-1])).grid(column=4,row=name_count + ROWS_ABOVE, padx=50)
            name_count +=  1
        ...
        customer_name_error_var.set("Check inputs")
        items_hired_error_var.set("")
